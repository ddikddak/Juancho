...
export type ExtraGatewayService = {
  ...
  detail: string;
  scope: "user" | "system";
  marker?: "juancho" | "openclaw" | "clawdbot" | "moltbot";
  legacy?: boolean;
};
...
const EXTRA_MARKERS = ["juancho", "openclaw", "clawdbot", "moltbot"] as const;

export function renderGatewayServiceCleanupHints(
  env: Record<string, string | undefined> = process.env as Record<string, string | undefined>,
): string[] {
  const profile = env.JUANCHO_PROFILE || env.OPENCLAW_PROFILE;
...
function hasGatewayServiceMarker(content: string): boolean {
  const lower = content.toLowerCase();
  const markerKeys = ["juancho_service_marker", "openclaw_service_marker"];
  const kindKeys = ["juancho_service_kind", "openclaw_service_kind"];
...
function isOpenClawGatewayLaunchdService(label: string, contents: string): boolean {
  ...
  return label.startsWith("ai.juancho.") || label.startsWith("ai.openclaw.");
}
...
function isOpenClawGatewaySystemdService(name: string, contents: string): boolean {
  ...
  if (!name.startsWith("juancho-gateway") && !name.startsWith("openclaw-gateway")) {
    return false;
  }
...
function isOpenClawGatewayTaskName(name: string): boolean {
  ...
  return (
    ...
    normalized.startsWith("juancho gateway") ||
...
async function scanLaunchdDir(params: {
  ...
  for (const entry of entries) {
    ...
    if (
      (marker === "juancho" || marker === "openclaw") &&
    ...
    results.push({
      ...
      scope: params.scope,
      marker,
      legacy: (marker !== "juancho" && marker !== "openclaw") || isLegacyLabel(label),
    });
  }
...
async function scanSystemdDir(params: {
  ...
  for (const entry of entries) {
    ...
    results.push({
      ...
      scope: params.scope,
      marker,
      legacy: marker !== "juancho" && marker !== "openclaw",
    });
  }
