...
- Workspace-only customization (zero src/ modifications, upstream sync maintained)
- Skills reference GSD templates by path (avoid content duplication, stay under size limits)
- nativeCommand frontmatter for Telegram commands (no separate wrapper skills needed for most commands)
- 30-minute heartbeat interval with UTC timezone as configurable defaults
- Moderate autonomy as default (intensive questioning at start, minimal interruption during execution)

**Issues Resolved:**

- Workspace file size limits (bootstrapMaxChars 20k) managed by condensing content and referencing templates by path
- Skill size limits (350 lines / 15k chars) managed by splitting into focused skills and referencing GSD workflows
- Content duplication avoided by path-based template references (@ syntax doesn't cross Task boundaries)
...
- None significant — workspace-only approach keeps codebase clean
